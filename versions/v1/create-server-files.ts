...
export const generateServer = (
  ...
  outputDirectory = fs.mkdtempSync(path.join(os.tmpdir(), "satsuma-"))
): string => {
  // Write the server.json
  fs.writeFileSync(
    path.join(outputDirectory, "server.config.json"),
    JSON.stringify(config)
  );

  // Write the satsuma config
  fs.writeFileSync(
    path.join(outputDirectory, ".satsuma.json"),
    JSON.stringify(metadata)
  );

  // copy resolvers.ts & typeDefs.ts from inputDirectory to outputDirectory
  const required = ["resolvers.ts", "typeDefs.ts"];
  ...
  fs.renameSync(
    path.join(outputDirectory, "pl.json"),
